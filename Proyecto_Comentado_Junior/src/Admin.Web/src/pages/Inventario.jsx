 ...
 * Este archivo Inventario.jsx es un componente funcional de React.
 * Representa la página del inventario en el panel de administrador.
 * 
 * Conceptos clave utilizados aquí:
 * 1. useState: Un "Hook" de React que permite guardar datos (estado) en la memoria del componente.
 *    Al cambiar el estado, React vuelve a pintar (renderizar) la interfaz automáticamente.
 * 2. useEffect: Un "Hook" que ejecuta código en un momento específico, generalmente cuando 
 *    el componente se muestra por primera vez en pantalla. Ideal para hacer peticiones iniciales.
 * 3. api.get / api.post / etc.: Uso de la librería Axios (configurada en '../api/client.js') 
 *    para comunicarse con el Backend (Controladores construidos en .NET).
 */
import { useState, useEffect } from 'react';
import api from '../api/client';

function Inventario() {
    // [EXPLICACIÓN PARA JUNIOR] Declaración de variables de estado:
    // "productos" guarda el array de datos de la API. "setProductos" es la función para actualizar ese valor.
    const [productos, setProductos] = useState([]);
    const [loading, setLoading] = useState(true); // Para mostrar un mensaje de "Cargando..."
    const [showModal, setShowModal] = useState(false); // Para mostrar u ocultar la ventana modal (formulario)
    const [editingProduct, setEditingProduct] = useState(null); // Guarda el producto si estamos editando, o null si creamos uno nuevo.
    ...
    // [EXPLICACIÓN PARA JUNIOR]
    // El useEffect con un arreglo vacío [] al final indica que esta función 
    // se ejecutará SOLO UNA VEZ cuando el componente se monte (cargue en el navegador).
    useEffect(() => {
        fetchProductos();
        fetchCategorias();
    }, []);
