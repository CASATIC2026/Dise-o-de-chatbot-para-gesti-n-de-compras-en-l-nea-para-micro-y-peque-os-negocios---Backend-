    ...
    return (
        <div className="animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-neutral-900 tracking-tight">Inventario</h1>
                    <p className="text-neutral-500 mt-2">Gestiona tu catálogo de productos con facilidad</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-neutral-50/50 border-b border-neutral-200">
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-neutral-500 uppercase tracking-wider">Precio</th>
                        ...
                        <tbody className="divide-y divide-neutral-100">
                            {filteredProductos.map((producto) => (
                                <tr key={producto.id} className="hover:bg-neutral-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            {producto.imagenUrl ? (
                                                <img src={producto.imagenUrl} alt={producto.nombre} className="w-10 h-10 rounded-lg object-cover border border-neutral-200" />
                                            ) : (
                                                <div className="w-10 h-10 rounded-lg bg-neutral-100 flex items-center justify-center border border-neutral-200 text-neutral-400">📷</div>
                                            )}
                                            <div>
                                                <div className="font-semibold text-neutral-900">{producto.nombre}</div>
                                                <div className="text-xs text-neutral-500 font-medium truncate max-w-[200px]">{producto.descripcion}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${producto.stock < 10 ? 'bg-red-50 text-red-700 border-red-200' : 'bg-neutral-50 text-neutral-700 border-neutral-200'
                                            }`}>
                                            {producto.stock} uds
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary-50 text-secondary-700 border border-secondary-100">
                                            {producto.categoria.nombre}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${producto.activo ? 'bg-green-50 text-green-700 border-green-200' : 'bg-neutral-100 text-neutral-600 border-neutral-200'
                                            }`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1.5">
                                            <button
                                                ...
                                                className="p-1.5 text-secondary-600 hover:bg-secondary-50 rounded-lg transition-colors border border-transparent hover:border-secondary-100"
                                                title="Ver Imagen"
                                            >
                                                👁️
                                            </button>
                                            ...
                                            <button
                                                ...
                                                className="p-1.5 text-neutral-500 hover:bg-neutral-100 rounded-lg transition-colors border border-transparent hover:border-neutral-200"
                                                title="Desactivar"
                                            >
                                                🚫
                                            </button>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
                        ...
                        <div className="p-6 overflow-y-auto">
                            <form id="productForm" onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    <label className="block text-sm font-semibold text-neutral-700 mb-1.5">Categoría <span className="text-red-500">*</span></label>
                                    <select
                                        className={`w-full px-4 py-2.5 bg-neutral-50 border ${hasError ? 'border-red-300 ring-1 ring-red-500/20' : 'border-neutral-200'} rounded-xl focus:bg-white focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all`}
                                    ...
                                    {hasError && (
                                        <p className="mt-1.5 text-xs text-red-600 font-medium flex items-center gap-1">
                                            <span>⚠️</span> Debe seleccionar una categoría para continuar.
                                        </p>
                                    )}
                                </div>
                                <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-700 mb-1.5">Precio <span className="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-700 mb-1.5">Stock <span className="text-red-500">*</span></label>
                                        <input
                                            type="number"
                                ...
                                <div className="flex items-center bg-neutral-50 p-3 rounded-xl border border-neutral-200">
                                    <input
                                        type="checkbox"
                                        id="activoCheck"
                                    ...
                                    />
                                    <label htmlFor="activoCheck" className="ml-3 text-sm font-semibold text-neutral-700 select-none cursor-pointer">Producto activo (visible en tienda)</label>
                                </div>
                            </form>
                        </div>
            ...
            {showImagePreview && (
                <div
                    className="fixed inset-0 bg-neutral-900/80 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-fade-in"
                    onClick={handleCloseImagePreview}
                >
                    <div className="relative bg-white rounded-2xl p-2 max-w-2xl w-full shadow-2xl" onClick={e => e.stopPropagation()}>
                        <button
                            className="absolute -top-12 right-0 text-white/70 hover:text-white text-3xl transition-colors bg-white/10 hover:bg-white/20 rounded-full w-10 h-10 flex items-center justify-center"
                            onClick={handleCloseImagePreview}
                        >
                            ✕
                        </button>
